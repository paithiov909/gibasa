...
- Refactored `tagger_impl` to improve performance of
  `tokenize(split = TRUE)`.
...
- `tokenize` now warns rather than throws an error when an invalid input
  is given during partial parsing. With this change, `tokenize` is no
  longer entirely aborted even if an invalid string is given. Parsing of
  those strings is simply skipped.
...
- Refactored `bind_tf_idf2`.
  - **Breaking Change**: Changed behavior when `norm=TRUE`. Cosine
    nomalization is now performed on `tf_idf` values as in the RMeCab
    package.
...
- Updated Makevars for Unix alikes. Users can now use a file specified
  by the `MECABRC` environment variable or `~/.mecabrc` to set up
  dictionaries.
...
- For performance, `tokenize` now skips resetting the output encodings
  to UTF-8.
...
- Use `RcppParallel::parallelFor` instead of `tbb::parallel_for`. There
  are no user’s visible changes.
...
- `tokenize` can now accept a character vector in addition to a
  data.frame like object.
- `gbs_tokenize` is now deprecated. Please use the `tokenize` function
  instead.
...
- Added the `partial` argument to `gbs_tokenize` and `tokenize`. This
  argument controls the partial parsing mode, which forces to extract
  given chunks of sentences when activated.
...
- gibasa now includes the MeCab source, so that users do not need to
  ...
  (to use `tokenize`, it still requires MeCab and its dictionaries
...
- `tokenize` now takes a data.frame as its first argument, returns a
  data.frame only. The former function that gets character vector and
  returns a data.frame or named list was renamed as `gbs_tokenize`.
