...
struct mecab_node_t {
   ...
   * This value is MECAB_NOR_NODE, MECAB_UNK_NODE, MECAB_BOS_NODE,
   * MECAB_EOS_NODE, or MECAB_EON_NODE.
   */
  unsigned char stat;
 ...
 */
MECAB_DLL_EXTERN char *mecab_sparse_tostr3(mecab_t *mecab, const char *str,
                                           size_t len, char *ostr, size_t olen);
 ...
 */
MECAB_DLL_EXTERN const char *mecab_nbest_sparse_tostr(mecab_t *mecab, size_t N,
                                                      const char *str);
...
/**
 * C wrapper of MeCab::Tagger::parseNBest(size_t N, const char *str, char *ostr,
 * size_t olen)
 */
 ...
 */
MECAB_DLL_EXTERN int mecab_nbest_init2(mecab_t *mecab, const char *str,
                                       size_t len);
 ...
 */
MECAB_DLL_EXTERN char *mecab_nbest_next_tostr2(mecab_t *mecab, char *ostr,
                                               size_t olen);
 ...
 */
MECAB_DLL_EXTERN const mecab_dictionary_info_t *mecab_dictionary_info(
    mecab_t *mecab);
 ...
 */
MECAB_DLL_EXTERN mecab_node_t *mecab_lattice_get_bos_node(
    mecab_lattice_t *lattice);

/**
 * C wrapper of MeCab::Lattice::eos_node()
 */
MECAB_DLL_EXTERN mecab_node_t *mecab_lattice_get_eos_node(
    mecab_lattice_t *lattice);
...
MECAB_DLL_EXTERN mecab_node_t **mecab_lattice_get_all_begin_nodes(
    mecab_lattice_t *lattice);
/**
 * C wrapper of MeCab::Lattice::end_nodes()
 */
MECAB_DLL_EXTERN mecab_node_t **mecab_lattice_get_all_end_nodes(
    mecab_lattice_t *lattice);

/**
 * C wrapper of MeCab::Lattice::begin_nodes(pos)
 */
MECAB_DLL_EXTERN mecab_node_t *mecab_lattice_get_begin_nodes(
    mecab_lattice_t *lattice, size_t pos);

/**
 * C wrapper of MeCab::Lattice::end_nodes(pos)
 */
MECAB_DLL_EXTERN mecab_node_t *mecab_lattice_get_end_nodes(
    mecab_lattice_t *lattice, size_t pos);

/**
 * C wrapper of MeCab::Lattice::sentence()
 */
MECAB_DLL_EXTERN const char *mecab_lattice_get_sentence(
    mecab_lattice_t *lattice);
 ...
 */
MECAB_DLL_EXTERN void mecab_lattice_remove_request_type(
...
MECAB_DLL_EXTERN const char *mecab_lattice_nbest_tostr2(
    mecab_lattice_t *lattice, size_t N, char *buf, size_t size);
 ...
 */
MECAB_DLL_EXTERN int mecab_lattice_get_boundary_constraint(
    mecab_lattice_t *lattice, size_t pos);

/**
 * C wrapper of MeCab::Lattice::feature_constraint(pos)
 */
MECAB_DLL_EXTERN const char *mecab_lattice_get_feature_constraint(
    mecab_lattice_t *lattice, size_t pos);

/**
 * C wrapper of MeCab::Lattice::boundary_constraint(pos, type)
 */
MECAB_DLL_EXTERN void mecab_lattice_set_boundary_constraint(
    mecab_lattice_t *lattice, size_t pos, int boundary_type);

/**
 * C wrapper of MeCab::Lattice::set_feature_constraint(begin_pos, end_pos,
 * feature)
 */
MECAB_DLL_EXTERN void mecab_lattice_set_feature_constraint(
    mecab_lattice_t *lattice, size_t begin_pos, size_t end_pos,
    const char *feature);
...
class MECAB_DLL_CLASS_EXTERN Lattice {
 public:
   ...
   * You can obtain all nodes via "for (const Node *node = lattice->bos_node();
   * node; node = node->next) {}"
   ...
   * You can obtain all nodes via "for (const Node *node =
   * lattice->end_nodes(pos); node; node = node->enext) {}"
   ...
   * You can obtain all nodes via "for (const Node *node =
   * lattice->begin_nodes(pos); node; node = node->bnext) {}"
   * @param pos position of nodes. 0 <= pos < size()
   * @return node linked list
   */
  ...
  /**
   * Return sentence.
   * If MECAB_NBEST or MECAB_PARTIAL mode is off, the returned poiner is the
   * same as the one set by set_sentence().
   * @return sentence
   */
   ...
   * Returned object is managed by this instance. When clear/set_sentence()
   * method is called, the returned buffer is initialized.
   * @return string representation of the lattice
   */
   ...
   */
  virtual void set_feature_constraint(size_t begin_pos, size_t end_pos,
...
class MECAB_DLL_CLASS_EXTERN Model {
 public:
   ...
   * meaning that passed |model| will no longer be accessible after calling this
   * method. return true if new model is swapped successfully. This method is
   * thread safe. All taggers created by Model::createTagger() method will also
   * be updated asynchronously. No need to stop the parsing thread explicitly
...
#ifndef SWIG
  /**
   * Factory method to create a new Model with a specified main's
   * argc/argv-style parameters. Return NULL if new model cannot be initialized.
   * Use MeCab::getLastError() to obtain the cause of the errors.
  ...
  /**
   * Factory method to create a new Model with a string parameter
   * representation, i.e.,
   * "-d /user/local/mecab/dic/ipadic -Ochasen".
   * Return NULL if new model cannot be initialized. Use MeCab::getLastError()
   * to obtain the cause of the errors.
   * @return new Model object
...
class MECAB_DLL_CLASS_EXTERN Tagger {
 public:
   ...
   * Return true if lattice is parsed successfully.
   * A sentence must be set to the lattice with Lattice:set_sentence object
   * before calling this method. Parsed node object can be obtained with
   * Lattice:bos_node. This method is thread safe.
   ...
   * Currently, N must be 1 <= N <= 512 due to the limitation of the buffer
   * size. You should not delete the returned string. The returned buffer is
   * overwritten when parse method is called again. This method is DEPRECATED.
   * Use Lattice class.
   * @param N how many results you want to obtain
   * @param str sentence
   * @return parsed result
   ...
   * @return parsed result
   */
  virtual const char *parse(const char *str, size_t len, char *ostr,
                            size_t olen) = 0;
   ...
   * @return parsed result
   */
  virtual const char *parseNBest(size_t N, const char *str, size_t len,
                                 char *ostr, size_t olen) = 0;
   ...
   * This method is DEPRECATED. Use Lattice::add_request_type(MECAB_ALL_MORPHS)
   * or Lattice::remove_request_type(MECAB_ALL_MORPHS)
   * @param all_morphs
   */
...
#ifndef SWIG
  /**
   * Factory method to create a new Tagger with a specified main's
   * argc/argv-style parameters. Return NULL if new model cannot be initialized.
   * Use MeCab::getLastError() to obtain the cause of the errors.
   * @return new Tagger object
   * @param argc number of parameters
  ...
  /**
   * Factory method to create a new Tagger with a string parameter
   * representation, i.e.,
   * "-d /user/local/mecab/dic/ipadic -Ochasen".
   * Return NULL if new model cannot be initialized. Use MeCab::getLastError()
   * to obtain the cause of the errors.
 ...
 * In some environment, e.g., MS-Windows, an object allocated inside a DLL must
 * be deleted in the same DLL too.
