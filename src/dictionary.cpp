...
int calcCost(const std::string &w, const std::string &feature, int factor,
  ...
  const CharInfo cinfo =
      property->getCharInfo(w.c_str(), w.c_str() + w.size(), &mblen);
...
struct pair_1st_cmp {
  bool operator()(const std::pair<T1, T2> &x1, const std::pair<T1, T2> &x2) {
    return std::less<T1>()(x1.first, x2.first);
  }
};
}  // namespace
...
void Dictionary::close() { dmmap_->close(); }
...
bool Dictionary::assignUserDictionaryCosts(const Param &param,
  ...
  CHECK_DIE(cid.left_size() == matrix.left_size() &&
      ...
      << "Context ID files(" << left_id_file << " or " << right_id_file
      << " may be broken: " << cid.left_size() << " " << matrix.left_size()
      << " " << cid.right_size() << " " << matrix.right_size();
...
bool Dictionary::compile(const Param &param,
  ...
  for (size_t i = 0; i < dics.size(); ++i) {
    ...
    if (!ifs) {
      if (type == MECAB_UNK_DIC) {
        Rcpp::Rcerr << dics[i] << " is not found. minimum setting is used."
                    << std::endl;
        is = &iss;
      } else {
        CHECK_DIE(ifs) << "no such file or directory: " << dics[i];
    ...
    while (is->getline(line.get(), line.size())) {
      ...
      if (cost == INT_MAX) {
        ...
        cost = calcCost(w, feature, factor, fi.get(), rewrite.get(),
                        property.get());
      }
      ...
      if (w.empty()) {
        Rcpp::Rcerr << "empty word is found, discard this line" << std::endl;
        continue;
      }

      if (!iconv.convert(&feature)) {
        Rcpp::Rcerr << "iconv conversion failed. skip this entry" << std::endl;
        continue;
      }
  ...
  CHECK_DIE(da.build(str.size(), const_cast<char **>(&str[0]), &len[0], &val[0],
                     &progress_bar_darts) == 0)
