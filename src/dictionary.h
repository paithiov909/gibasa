...
class Dictionary {
 public:
  ...
  size_t commonPrefixSearch(const char *key, size_t len, result_type *result,
                            size_t rlen) const {
  ...
  bool isCompatible(const Dictionary &d) const {
    return (version_ == d.version_ && lsize_ == d.lsize_ &&
            rsize_ == d.rsize_ &&
  ...
  explicit Dictionary()
      : dmmap_(new Mmap<char>), token_(0), feature_(0), charset_(0) {}
