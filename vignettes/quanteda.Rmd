...
corp <- corp |>
  dplyr::mutate(
    body = stringi::stri_trans_nfkc(body) |>
      stringr::str_remove_all("(https?\\://[[:alnum:]\\.\\-_/]+)"),
  ) |>
  ...
  gibasa::mute_tokens(token %in% quanteda::stopwords("ja", "marimo")) |>
...
corp |>
  ...
  quanteda.textplots::textplot_wordcloud(color = viridisLite::turbo(8))
```
...
```{r ca}
corp_sample <- quanteda::corpus_sample(corp, size = 24L)
corp_sample |>
  quanteda::tokens(
    what = "fastestword",
    remove_punct = TRUE,
  ...
  quanteda::dfm() |>
  quanteda::dfm_tfidf() |>
  quanteda.textmodels::textmodel_ca(sparse = TRUE) |>
...
corp_sample |>
  ...
  quanteda::dfm() |>
  quanteda::dfm_trim(max_termfreq = 300L) |>
  quanteda::dfm_group(groups = category) |>
...
ggplot2::ggplot(ggdendro::segment(d)) +
  ...
  ggplot2::geom_text(
    ggdendro::label(d),
    mapping = aes(x, y, label = names, colour = category, hjust = 0),
    size = 3
  ) +
  ggplot2::coord_flip() +
...
なお、トピック数は9に決め打ちしています。トピック数含めパラメタの探索をしたい場合には、[ldatuning](https://github.com/nikita-moor/ldatuning)や[stm](https://github.com/bstewart/stm)などを利用できます。
